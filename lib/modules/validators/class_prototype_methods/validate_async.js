...
import callAsyncMeteorMethod from '../../storage/utils/call_async_meteor_method';

async function validateAsync (options = {}, callback) {
  const doc = this;
  ...
  if (connection && connection !== Meteor.server) {
    ...
    try {
      // Run Meteor method.
      return await callAsyncMeteorMethod(
